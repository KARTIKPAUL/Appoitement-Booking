...
const getAllCourses = async function (req, res, next) {
  try {
    ...
    return next(
      new AppError(`Fetching Course failed !!,${error.message}`, 400)
    );
  }
};
...
const createCourse = async function (req, res, next) {
  try {
    ...
    return next(
      new AppError(
        `Something Went Wrong When Creating Cousre ! ${error.message}`,
        501
      )
    );
  }
...
const addLectureToCourseById = async function (req, res, next) {
  try {
    ...
    if (!title || !description) {
      return next(
        new AppError(
          "Title And Description Are Missing When Uploaded lecture",
          400
        )
      );
    }
